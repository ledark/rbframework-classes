...
class Chance {
    ...
    /**
     * phone function retonar um telefone aleatório no formato (XX) ?XXXX-XXXX
     * Você também pode retornar apenas os númer com a flag $onlyNumbers = true
     *
     * @param boolean|null $mobile
     * @param boolean $onlyNumbers
     * @return string
     */
    public static function phone(bool $mobile = null, bool $onlyNumbers = false):string {
        $ddd = rand(11, 99);
        if(is_null($mobile)) {
            $phoneStart = self::pickOne(['', '9']);
        } else {
            $phoneStart = ($mobile) ? '9' : '';
        }
        $phoneA = rand(1000, 9999);
        $phoneB = rand(1000, 9999);
        return ($onlyNumbers) ? $phoneStart.$phoneA.$phoneB : "({$ddd}) {$phoneStart}{$phoneA}-{$phoneB}";
    }
    ...
    public static function user() {
        ...
        return array(
            ...
            'endereco' => Http::getJsonResponse('http://midiacriativa.com/cep/busca-json.php?cep=generate'),
        );
    }
    ...
    public static function cnpj(bool $formated = true) {
        ...
        $d2 = 11 - ($d2 % 11);
        if($d2 >= 10) $d2 = 0;
        $strFormated = $n1.$n2.'.'.$n3.$n4.$n5.'.'.$n6.$n7.$n8.'/'.$n9.$n10.$n11.$n12.'-'.$d1.$d2;
        if($formated) {
            return $strFormated;
        } else {
