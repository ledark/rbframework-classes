...
class Model {
    ...
    private function parseMysql(string $mysqlcode):array {
        $parsed = ['sql' => $mysqlcode];
        if(strpos($mysqlcode, ' PRIMARY')) {
            $parsed['sql'] = explode(' PRIMARY', $mysqlcode);
            $parsed['sql'] = reset($parsed['sql']);
        }
        if(strpos($mysqlcode, ' UNIQUE')) {
            $parsed['sql'] = explode(' UNIQUE', $mysqlcode);
            $parsed['sql'] = reset($parsed['sql']);
        }
        return $parsed;
    }
