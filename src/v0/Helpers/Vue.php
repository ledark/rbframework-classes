...
class Vue {
    ...
    public function replaces($global_replaces) {
        if(is_array($global_replaces)) {
            $this->global_replaces = array_merge($this->global_replaces, $global_replaces);
        }
        return $this;
    }
        ...
        foreach($this->search_paths as $path) {
            foreach($this->search_extensions as $ext) {
                if(file_exists($path.$name.$ext)) {
                     if($this->verbose) echo "<script>console.info(\"O arquivo VUE [{$path}{$name}{$ext}] foi incluído com sucesso!\");</script>";
                    $this->addFileComponent($path.$name.$ext, $replaces);
                    return $this;
                } else {
                    if($this->verbose) echo "<script>console.error(\"O arquivo VUE [{$path}{$name}{$ext}] não pode ser incluído\");</script>";
                }
            }
        }
        echo "<script>console.error(\"Nenhum arquivo VUE [$name] não pode ser incluído\");</script>";
        return $this;
    }
